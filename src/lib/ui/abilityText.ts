...
export const ABILITY_ROLE: Record<string, string> = {
  ...
  riposte:
    'Венец Оплота: тяжёлый удар щитом, после которого следующие два удара врага мягче.',
  'shield-jab':
    'Заполнитель наоборот: бьёт слабее «Скорого выпада», зато каждый раз ослабляет цель.',
  'second-wind':
    'Венец Бдения: разом наливает треть запаса. Не про урон — про отменённый привал.',
  'blunt-edge':
    'Череполом наоборот: бьёт сразу, стоит фиксированно и входит в каждый цикл.',
  'endless-reckoning':
    'Венец Резни: добивание с коротким откатом, возвращающее большую часть ярости.',
  butcher:
    'Венец Жил: платит здоровьем за ярость вдвое щедрее и входит в стойкость тем же ударом.',
  call: 'Венец Чутья: одним нажатием наливает полоску доверху. Против пустого начала боя.',
  'pack-release':
    'Венец Гона: десять секунд пёс кусает вдвое чаще и вдвое тяжелее — работает за двоих.',
  swap: 'Венец Привязи: пять секунд входящее уходит в пса, и он же получает лечение.',
  breather: 'Венец Тропы: первое лечение у Псаря. Пёс сторожит, пока герой переводит дух.',
  // --- Изувер ---
