    ...
    {#each all as ability (ability.id)}
      ...
      >
        ...
        <span class="name">
          ...
          <!-- ПРАВЛЕНО ТАЛАНТАМИ — ВИДНО БЕЗ НАВЕДЕНИЯ. Книга показывает
               эффективное умение, и без метки игрок не отличил бы «так и
               было» от «это мой талант». Эффективное умение без правок —
               ТОТ ЖЕ объект, что базовое; иного признака не нужно. -->
          {#if ability !== ABILITY_BY_ID[ability.id]}
            <Tag tone="xp" label="талантами" />
          {/if}
        </span>
