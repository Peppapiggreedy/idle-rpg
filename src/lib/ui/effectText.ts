...
const WHAT: Record<EffectView['kind'], string> = {
  ...
  // ЧТО ИМЕННО ДАЁТ ПРОК, СКАЗАТЬ ОДНОЙ СТРОКОЙ НЕЛЬЗЯ: у каждого таланта своя
  // характеристика и своё окно. Строка называет РОД («сработал талант»), а
  // числа игрок читает у самого таланта — там они и лежат.
  proc: 'Сработал талант: прибавка держится окном',
}
