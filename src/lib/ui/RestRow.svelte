...
<style>
  ...
  /* ПОЛЗУНОК НЕ ИМЕЕТ ПРАВА СЖАТЬСЯ ДО НИТКИ. С `min-width: 0` он отдавал
     всю ширину соседям по строке и при узкой полосе становился в пятнадцать
     пикселей: формально видимый, взять его нечем. Замер на полном наборе
     склянок (пресет `potions`, 1280px): полоса 282px, из них подпись 75,
     число 72, подсказка 70, отступы и зазоры 48 — ползунку оставалось 15. */
  input {
    flex: 1 1 8rem;
    min-width: 6rem;
  ...
  /* Подсказка — последней и первой же уходит: ползунок и число нужны всегда,
     а подсказка — нет. Раньше это было обещанием на словах: `flex: 0 0 auto`
     с `nowrap` не отдавал ни пикселя, и уходил не он, а ползунок. Теперь
     подсказка ДЕЙСТВИТЕЛЬНО уступает — сжимается до нуля, когда полосе тесно. */
  .hint {
    display: none;
    flex: 0 1 auto;
    min-width: 0;
