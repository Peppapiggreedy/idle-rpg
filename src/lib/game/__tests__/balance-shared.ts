...
export const COLUMNS =
  'зона                 убийств/ч   золота/ч    опыта/ч смертей/ч  до ур.  итог    доход/ч'

/**
 * ДОХОД ЗА ЧАС В ЭТОЙ ЗОНЕ: убийства ИЗ ПРОГОНА, цена убийства ИЗ ДАННЫХ.
 *
 * ЗАЧЕМ ГИБРИД, А НЕ ПРОСТО `goldPerHour` ПРОГОНА. С переносом семидесяти
 * процентов дохода на продажу находок золото прогона стало тяжелохвостой
 * случайной величиной: разброс ОДНОЙ находки — 143 % от среднего (легендарка
 * стоит тридцать обычных при вероятности 0.6 %). За четыре часа это ±9 %, а
 * СОСЕДНИЕ ЗОНЫ отличаются доходом на те же ~10 % — и строгая монотонность
 * по двадцати зонам начала падать на шуме: замер дал три инверсии по 1-2 %
 * (луг → каменоломня, терраса → перевал, гряда → криволесье) при ровной
 * кривой в остальном. Догнать точность часами нельзя: нужно тридцатикратное
 * окно.
 *
 * Поэтому из замера убран РОВНО ОДИН бросок — рулетка тира, — и заменён своим
 * матожиданием. Всё остальное по-прежнему меряется настоящим тиком: сколько
 * герой успевает убить, кого ему выдаёт спавн, сколько он простаивает на
 * привалах и умирает. Цена убийства берётся оттуда же, откуда её берёт модель
 * дохода (`goldPerHourAt`), — второй формулы не появляется.
 *
 * Что при этом перестало проверяться: что рулетка тиров платит по своей
 * таблице. Это держат тесты быстрого набора (`loot.test.ts`: цена растёт от
 * уровня и тира, доли крана сходятся на каждой зоне) и сверка модели с
 * прогоном в `gold.test.ts` — она осталась на настоящем золоте.
 */
export function zoneIncomePerHour(result: SimResult, zone: Zone): Decimal {
  const typical = representativeMonster(zone)
  const perKill = typical.goldReward.plus(averageItemSellPrice(typical.level).times(DROP_CHANCE))
  return result.killsPerHour.times(perKill)
}
