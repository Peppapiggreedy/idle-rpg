...
describe('фикстуры сейвов', () => {
  ...
  it('save-v15: связка двух рук переживает загрузку', () => {
    ...
    // Сила блока — сумма щита и вклада выносливости: щит задаёт базу, атрибут
  ...
  it('save-v11: счётчик сбросов переживает загрузку, ранги переносятся до 29-й', () => {
    ...
    // «ОСТРЫЙ ГЛАЗ» ПО ДОРОГЕ ОТВАЛИЛСЯ, И ЭТО РАБОТА СТОРОЖА, А НЕ ПОТЕРЯ.
    // Шаг 18→19 переносит ранг только туда, куда он ДОСТИЖИМ: этаж обязан
    // быть открыт тем, что удержано выше. В старом дереве «Острый глаз» стоял
    // на втором этаже при пороге 5, и пяти очков «Отточенного клинка» хватало;
    // в семиэтажном Гневе тот же второй этаж стоит десяти, и два очка ложатся
    // за закрытый порог. Очки при этом не пропадают: доступные считаются как
    // «заработано минус вложено», и ниже это проверено числом.
