...
//
// Ревизия (docs/archive/AUDIT-2025-09.md) насчитала двенадцать правил, которые не уронит ни один
...
export const TECHNICAL_CONSTANTS: Record<string, string> = {
  ...
  // СЕМЬ КОНСТАНТ НИЖЕ ОТКРЫЛИСЬ, КОГДА СТОРОЖ НАУЧИЛСЯ ВИДЕТЬ `export const`.
  // До этого регулярка требовала, чтобы строка начиналась с `const`, и всё
  // экспортированное проезжало мимо. Ни одна из семи не оказалась балансной —
  // дырой не воспользовались, — но каждая названа здесь с причиной, иначе
  // список исключений превратится в свалку «чтобы тест молчал».
  'formulas.ts:MAX_LEVELUPS_PER_CALL':
    'предохранитель от зависания на абсурдном опыте: сколько уровней взять за один вызов',
  'loop.ts:STEP_MS':
    'шаг симуляции, свойство ЦИКЛА: все игровые величины считаются в секунду и от него не зависят',
  'loop.ts:MAX_STEPS_PER_FRAME':
    'защита от спирали смерти после заморозки вкладки — свойство цикла, а не игры',
  'save.ts:SAVE_VERSION':
    'номер формата сейва: к балансу отношения не имеет, поднимается миграцией',
  'save.ts:FIRST_TREE_GENERATION':
    'каким поколением дерева помечены сейвы до 33-й версии: свойство ФОРМАТА, номера поколений живут в data/classes.ts',
  'save.ts:OFFLINE_MODAL_MIN_MS':
    'с какого простоя ПОКАЗЫВАТЬ модалку возврата: порог показа, а не начисления (само начисление считается от первой миллисекунды)',
  'simulate.ts:PACING_TAIL_LEVELS':
    'сколько последних уровней печатает таблица прогона: параметр ПРИБОРА, в игру не входит',
  'state.ts:COMBAT_LOG_SIZE':
    'длина хвоста лога в состоянии: сколько событий держать, а не сколько чего в игре',
}

/**
 * ОБЪЯВЛЕНИЕ ЧИСЛОВОЙ КОНСТАНТЫ — ОДИН ОБРАЗЕЦ НА ДВА МЕСТА. Его читают и
 * сторож баланса, и проверка «в списках исключений нет мёртвых записей»;
 * разойдись они — вторая начнёт считать живые исключения мёртвыми. Ровно это
 * и случилось: обе требовали, чтобы строка НАЧИНАЛАСЬ с `const`, и всё
 * экспортированное было им невидимо.
 */
const CONST_NUMBER = /^(?:export\s+)?const\s+([A-Z][A-Z0-9_]*)\s*(?::\s*[\w<>]+\s*)?=\s*(-?[\d_.]+)\s*$/

export function findBalanceNumbers(file: string, source: string): string[] {
  const out: string[] = []
  const code = stripComments(source)
  for (const line of code.split('\n')) {
    const m = line.trim().match(CONST_NUMBER)
    if (!m) continue
    const key = `${file}:${m[1]}`
...
export const ID_BRANCH_ALLOWED: Record<string, string> = {
  ...
  // ОМОНИМ, А НЕ ВЕТВЛЕНИЕ: `entry.kind === 'armor'` разбирает ВИД стартового
  // предмета (броня / оружие), а `armor` заодно оказался именем стата. Это
  // разные словари, и проверка видит совпадение строк, а не ветку по стату:
  // ни одного `if (стат === ...)` в state.ts нет и быть не должно.
  'state.ts:armor': 'вид стартового предмета, а не стат брони — совпали имена',
...
// в этом же проекте (см. docs/archive/AUDIT-2025-09.md, находка 6.1).
describe('проверки ловят заведомо битый код', () => {
  ...
  it('в списках исключений нет мёртвых записей', () => {
    ...
    for (const [file, src] of sources()) {
      ...
      for (const line of code.split('\n')) {
        const c = line.trim().match(CONST_NUMBER)
