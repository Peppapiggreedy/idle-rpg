...
/**
 * ПОЛ доли привалов. Контракт CLAUDE.md — 33 % и 36 % по классам; пол взят
 * заметно ниже (20 %), потому что его задача не повторить запись, а поймать
 * ИСЧЕЗНОВЕНИЕ привала: разница между «дороже, чем записано» и «привала нет»
 * — это разные игры, а один потолок их не различал.
 */
const REST_SHARE_MIN = 0.2
  ...
  for (const cls of CLASS_SET) {
    classIt(cls)(
      ...
      () => {
        ...
        // ПОЛ, А НЕ ТОЛЬКО ПОТОЛОК. Контракт из CLAUDE.md звучит числом
        // («полный путь 1..100 — 33 % и 36 % по классам»), а держал его один
        // потолок 0.4: и 27 %, и 0.007 % проходили его одинаково успешно.
        // Односторонняя проверка не отличает «привал стоит дорого» от
        // «привала нет вовсе», а это ровно два разных состояния игры.
        //
        // РАСХОЖДЕНИЕ ИЗВЕСТНО И НЕ РОНЯЕТ ПРОГОН — намеренно, тем же
        // приёмом, что и у превью-класса: замер даёт Стражу 27.2 % против
        // записанных 33 %, а Изуверу 0.007 % против 36 %. Ронять на этом
        // прогон значило бы либо подогнать коридор под факт (и контракт
        // перестал бы что-либо означать), либо перебалансировать привал
        // походя. И то и другое — работа балансовой ночи, а не сторожа.
        // Пока он ГОВОРИТ: печатает разрыв и не даёт ему вырасти молча.
        const share = medianOf(classId, (x) => x.restShare)
        if (share < REST_SHARE_MIN) {
          console.warn(
