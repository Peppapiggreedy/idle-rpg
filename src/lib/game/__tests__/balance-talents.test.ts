...
describe('ветки талантов', () => {
  for (const cls of CLASS_SET) {
    classIt(cls)(
      ...
      // Три полных билда на эталоне: ~830 с, когда тест идёт на машине один,
      // и ~965 с, когда четыре потока заняты честно (см. комментарий о сроках
      // в balance-zones.test.ts). Срок втрое от замера — запас на чужой раннер.
      1_800_000,
    )
  }
})
