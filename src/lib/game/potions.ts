 ...
 *
 * В РЯД ПОПАДАЕТ ТОЛЬКО ТО, ЧЕГО ЕСТЬ ХОТЯ БЫ ОДНА ШТУКА. Не «известен
 * рецепт», не «существует в данных», не «механика открыта» — есть в мешке.
 *
 * Раньше стояло «открылись зелья — показываем все девять», и довод был
 * такой: кнопка, которая прыгает по ряду, стоит игроку промаха. Довод верный,
 * но цена оказалась выше. Ряд действий ОДИН на умения и склянки: четыре
 * умения плюс девять склянок — тринадцать кнопок, из которых на телефоне
 * видно пять. Ряд распухал сразу на сороковом уровне, и распухал
 * НЕСУЩЕСТВУЮЩИМ: восемь из девяти склянок игрок ни разу не варил.
 *
 * Прыжка при этом меньше, а не больше: набор склянок в мешке меняется от
 * варки к варке, то есть раз в десятки минут, а не в каждом бою.
 *
 * ДЕЙСТВУЮЩАЯ СКЛЯНКА ОСТАЁТСЯ В РЯДУ, даже когда выпита последняя: у неё
 * идёт отсчёт, и убрать шкалу вместе с последней склянкой значило бы спрятать
 * то, что прямо сейчас работает.
 *
 * Порога уровня здесь больше нет, и он не нужен: склянки берутся только
 * варкой, а варка заперта своим порогом. Пустой мешок — пустой ряд.
 */
export function potionSlots(state: GameState): PotionSlot[] {
  return POTION_RECIPES.map((recipe) => potionStatus(state, recipe)).filter(
    (slot) => slot.active || slot.count.gt(0),
  )
}
