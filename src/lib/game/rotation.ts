...
function fundPlan(
  ...
  // ПАССИВНЫЕ — КАСТЫ С НУЛЕВЫМ ТЕМПОМ, И БЕРУТСЯ ОНИ СО ВСЕГО РЯДА.
  //
  // Нулевой темп — потому что их не жмут: ни маны, ни урона, ни замаха они
  // не стоят, и всё, что модель считает через аптайм, у них обнуляется само.
  // Но в `rot.casts` они обязаны БЫТЬ: `abilityMods` читает флаги именно
  // оттуда, а у пассивного флага («Свора» — второй пёс) действие от темпа не
  // зависит вовсе.
  //
  // СО ВСЕГО РЯДА, А НЕ ПО ГАЛКЕ АВТОКАСТА: пассивное работает тем, что
  // стоит в слоте, и галка «жать самому» к нему отношения не имеет. Читай
  // модель их по `onlyAutocast`, она теряла бы второго пса ровно у того
  // игрока, который галку снял, — а тик (`houndCapacity`) его выдаёт.
  for (const ability of abilitiesByPriority(rotation, false)) {
  ...
  for (const ability of abilitiesByPriority(rotation, plan.onlyAutocast)) {
    // Пассивное не жмётся ни рукой, ни автокастом: оно уже учтено выше
    // нулевым кастом. Пройди оно дальше — откат ноль дал бы ему темп
    // «раз в задержку реакции», то есть пять пустых кастов в секунду.
