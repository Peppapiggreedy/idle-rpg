...
describe('данные умений', () => {
  it('у каждого класса свои умения, у каждого цена, кулдаун и доля удара', () => {
    ...
    for (const a of ABILITIES) {
      ...
      // Бесплатных умений ровно три рода, и каждый — по правилу схемы:
      // «Сосредоточение» не платит за собственную скидку, генератор ресурса —
      // за собственную прибавку, а плата здоровьем платит ЗДОРОВЬЕМ.
      expect(a.manaCost.gt(0), a.id).toBe(!a.freeCasts && !a.generate && !a.bloodPrice)
