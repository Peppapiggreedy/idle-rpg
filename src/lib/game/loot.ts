...
/**
 * ВЕЩЬ ОТДАЁТ ДОЛЮ СВОИХ СТАТОВ — так оплачивается свойство сборки
 * (`data/boons.ts`). Считается ЗДЕСЬ, рядом с зерном, и через то же зерно:
 * скидка, оставившая после себя дробную силу там, где сила считается штуками,
 * развела бы карточку с боем.
 *
 * БАЗОВЫЕ МОДИФИКАТОРЫ НЕ ТРОГАЮТСЯ. `kind: 'base'` не прибавляет, а
 * ЗАМЕНЯЕТ умолчание: срезать долю со скорости оружия значило бы сделать его
 * быстрее, то есть заплатить прибавкой. База — это образец вещи, а не её
 * сила.
 */
export function discounted(mods: StatModifier[], share: number): StatModifier[] {
  if (!(share > 0)) return mods
  const keep = 1 - share
  return grained(
    mods.map((mod) =>
      mod.kind === 'base' ? mod : { ...mod, value: mod.value.times(keep) },
    ),
  )
}
