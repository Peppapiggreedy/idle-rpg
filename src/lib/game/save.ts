...
export const MIGRATIONS: Record<number, (raw: RawSave) => RawSave> = {
  ...
  // ветки выросла с 61 до 115-118, а числа срезаны в 0.586 раза под ту же
  // строку бюджета. Ни один id не исчез — вложенные ранги ЛЕГЛИ БЫ как есть,
  // и формально чинить нечего. Но это была бы честность буквы, а не дела:
  // игрок выбирал из другого дерева, и половины выборов в его сборке просто
  // не существовало.
  //
  // Поэтому ранги обнуляются, а очки возвращаются свободными — отдельного
  // счётчика доступных очков нет, они считаются как «заработано минус
  // вложено», так что достаточно убрать ранги.
  //
  // СЧЁТЧИК ПЛАТНЫХ СБРОСОВ НЕ ТРОГАЕТСЯ, и это главное в этой миграции.
  ...
  29: (raw) => ({
    ...
    version: 30,
    talents: {},
    talentResets: typeof raw.talentResets === 'number' ? raw.talentResets : 0,
  }),
