...
describe('правило чисел держится на всех восьми данжах', () => {
  function heroFor(
    ...
    const ready = ensureStats({
      ...
      level: new Decimal(level),
      // ГЕРОЙ КОНТРАКТА — ПЕРВЫЙ ПУТЬ ВЕТКИ, НЕ ГЛУБЖЕ ВЕНЦА. Контракт
      // меряет ЧИСЛА БОССОВ против базового героя, и герой обязан стоять на
      // месте, когда правят дерево — иначе непонятно, что поехало.
      //
      // ПОТОЛОК ГЛУБИНОЙ. Пока ветка была лестницей, её ёмкость (61) сама
      // обрезала вложение; с ёмкостью 115 те же очки помещались все, и замер
      // упал с 78.6 % до 69.4 при неизменных боссах.
      //
      // КЛЮЧЕВЫЕ ВХОДЯТ. Проверялся и герой без ключевых: у него первый босс
      // седьмого тира стоит 95.7 %, а восьмой тир не проходится вовсе —
      // боссы калибровались на герое, у которого на 60 очках ключевые пятого
      // и девятого этажей были. Куплены они по порядку первого пути, и
      // порядок этот — часть калибровки: «Рваный выпад» стоит в «Крови»
