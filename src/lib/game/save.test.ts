...
// ЭТОТ БЛОК ПОЯВИЛСЯ ИЗ ПОЛОМКИ (находка 2.1 в docs/archive/AUDIT-2025-09.md). Игра штамповала
...
describe('версия сейва', () => {
  it('29 -> 30: дерево пересобрано — сброс бесплатный, счётчик НЕ РАСТЁТ', () => {
    // ГЛАВНОЕ В ЭТОЙ МИГРАЦИИ — ИМЕННО СЧЁТЧИК. Цена следующего сброса растёт
    // от `talentResets`; подними его здесь — и подарок обернулся бы
    // подорожанием, то есть был бы не подарком, а отложенным счётом.
    const next = MIGRATIONS[29]({
      version: 29,
      talents: { 'wrath-honed-edge': 6, 'bulwark-thick-hide': 3 },
      talentResets: 2,
      gold: '5',
    } as never)
    expect(next.version).toBe(30)
    // Ранги убраны — очки вернулись свободными: отдельного счётчика
    // доступных очков нет, они считаются как «заработано минус вложено».
    expect(next.talents).toEqual({})
    expect(next.talentResets).toBe(2)
    // Всё остальное миграция не трогает.
...
// ПРОГРЕСС НЕЛЬЗЯ ПОТЕРЯТЬ МОЛЧА (находки 2.2, 2.3, 2.4 в docs/archive/AUDIT-2025-09.md).
//
