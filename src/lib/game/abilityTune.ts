...
export function tuneAbility(
  ...
  // ТИП ПРАВИТСЯ ТОЛЬКО МЕЖДУ ДВУМЯ АКТИВНЫМИ, И `passive` СЮДА НЕ ВХОДИТ
  // НАМЕРЕННО. Пассивность — это форма умения ЦЕЛИКОМ: четыре нуля (цена,
  // откат, урон, общая задержка) стоят вместе, и одна правка одного поля
  // `type` их не поставит. Талант, сделавший «Череполом» пассивным, оставил
  // бы ему цену и откат — то есть кнопку, которую нельзя нажать и которая
  // при этом чего-то стоит. В обратную сторону так же: активная «Свора» без
  // цены и отката была бы бесплатным мгновенным умением.
