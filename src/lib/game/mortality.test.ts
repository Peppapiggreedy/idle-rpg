...
describe('цена входа в бой: с полного запаса и с остатка', () => {
  ...
  for (const cls of CLASSES) {
    ...
    // ЛЕСТНИЦА СМЕРТНОСТИ ВЫРОВНЯЛАСЬ, и это прямое следствие спавна вокруг
    // уровня героя. Пока зона выдавала любой из пяти своих уровней, новичок
    // зоны в пятой части боёв встречал моба на четыре уровня старше себя, и
    // худший бой стоил ему 15-42 % запаса: 38 % на стартовом лугу против
    // 15-20 % на глубине. Теперь ему выдают +2 уровня максимум, и замер даёт
    // 18.0-25.8 % ПО ВСЕМ ДВАДЦАТИ ЗОНАМ — коридор вчетверо уже прежнего.
    //
    // Коридор 15-30 % с запасом по обе стороны: сузить его до замера значило
    // бы ронять прогон от любой правки урона мобов на процент.
    cit(`${cls.name}: цена худшего боя ровная по всей лестнице`, () => {
      for (const { zone, classId, share } of worstShares(cls.id)) {
        expect(share, `${classId} @ ${zone}`).toBeGreaterThan(0.15)
        expect(share, `${classId} @ ${zone}`).toBeLessThan(0.3)
      }
    })
    ...
    cit(`${cls.name}: войти с остатком запаса — реальная смерть, а не испуг`, () => {
      ...
      // ЗДЕСЬ СТОЯЛО «А В СТАРТОВОЙ ЗОНЕ ХУДШИЙ БОЙ СТОИТ БОЛЬШЕ ТРЕТИ
      // ЗАПАСА» — и это была правда ровно до спавна вокруг уровня героя.
      // Новичок в одном белом клинке встречал на лугу мобов ВПЯТЕРО старше
      // себя, и стартовая зона была самой дорогой схваткой всей лестницы
      // (38 % против 15-20 % на глубине). Теперь ему выдают мобов первого,
      // второго и третьего уровня, и луг встал в один ряд со всеми: 24.1 %
      // при коридоре 18-26 %.
      //
      // Утверждение не ослаблено, а ЗАМЕНЕНО на верное и более сильное:
      // «цена худшего боя ровная по всей лестнице» проверяется отдельным
      // тестом выше, и коридор там вчетверо уже прежнего.
      const first = shares[0]
      const deepest = shares[shares.length - 1]
      expect(
        Math.abs(first.share - deepest.share),
        `стартовая зона (${(first.share * 100).toFixed(1)} %) больше не выделяется ` +
          `на фоне последней (${(deepest.share * 100).toFixed(1)} %)`,
      ).toBeLessThan(0.12)
    })
  }
})
