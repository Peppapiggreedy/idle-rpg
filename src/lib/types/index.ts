...
export interface Item {
  ...
  procId?: string
  /** СВОЙСТВО СБОРКИ (data/boons.ts): вещь правит УМЕНИЕ, а не статы. Тот же
   *  вид эффекта, что у таланта, и тот же конвейер (game/abilityTune.ts).
   *  Оплачено долей собственных статов вещи — сумма нулевая. Как и у прока,
   *  здесь только id: числа лежат в данных одним экземпляром. */
  boonId?: string
}
...
export type CombatEvent =
  ...
  // Дерево талантов: ключевой этаж ветки стал доступен (порог набран) и
  // ключевой талант взят. Оба — строки журнала, без окон и пауз: игрок
  // обязан узнать, что выбор появился, но игра его не останавливает.
  | { type: 'talent-floor'; branchId: string; row: number }
  | { type: 'talent-key'; talentId: string }
  | { type: 'loot'; item: Item }
  ...
  // Разбор на лету распылил лишнюю находку. Свой код, а не 'autosell':
  // пыль — не золото, и в логе игрок обязан видеть, что именно он получил.
  | { type: 'autodust'; item: Item; dust: Decimal }
  ...
  // Выученный рецепт: событие, а не окно. Рецепт приходит посреди боя, и
  // останавливать ради него игру нельзя — но и промолчать нельзя тоже.
