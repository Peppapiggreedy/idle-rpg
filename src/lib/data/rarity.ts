...
/**
 * Ожидаемый множитель ЦЕНЫ по той же рулетке: Σ(вес × sellMult) / Σвес.
 *
 * Ради него и вынесено: доля золота с находок (GOLD_SOURCE_SHARE.loot)
 * задаётся на СРЕДНЮЮ находку, а средняя находка — это рулетка, а не
 * обычный предмет. Впишешь число руками — и первая же правка весов тира
 * молча уведёт кран золота, оставив формулу формально верной.
 */
export const EXPECTED_SELL_MULT: Decimal = RARITIES.reduce(
  (sum, r) => sum.plus(r.sellMult.times(r.weight)),
  new Decimal(0),
).div(RARITIES.reduce((sum, r) => sum + r.weight, 0))
