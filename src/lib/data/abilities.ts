...
export const ABILITIES: AbilityDef[] = [
  ...
  {
    // ОБУХ — умения нет ни в одной книге класса: его кладёт в ряд ТАЛАНТ
    // ЗАМЕНЫ («Тупая сторона»), вместо «Череполома», и играется оно под ЕГО
    // идентификатором.
    //
    // ЧЕМ ОТЛИЧАЕТСЯ — ФОРМОЙ, А НЕ ЧИСЛАМИ. «Череполом» ждёт замаха и тратит
    // ВСЮ полоску (`spendAll`), то есть это удар «всё или ничего» и ротацией
    // он не является. «Обух» бьёт сразу, стоит фиксированных сорока ярости и
    // растёт от полоски вдвое слабее: тот же вопрос класса («в каком я
    // сейчас»), но кнопка, которую жмут каждый цикл. Правкой полей из
    // `ABILITY_TUNABLE` этого не собрать: `spendAll` не поле, а форма.
    //
    // ЧИСЛА СВЕДЕНЫ ПО ЦЕНЕ РЕСУРСА, А НЕ ПО УРОНУ. «Череполом» на среднем
    // заполнении бьёт 1.5 + 10 × 0.5 = 6.5 за всю полоску (50) раз в 12 с —
    // 0.130 удара оружия на единицу ярости и 4.17 ярости в секунду. «Обух»:
    // 2.1 + 5 × 0.5 = 4.6 за 20 раз в 6 с — 0.230 на единицу и 3.33 в
    // секунду, то есть ДЕШЕВЛЕ И ПО ПОТОКУ, И ПО ЕДИНИЦЕ.
    //
    // ЗАМЕР НАЗВАЛ ПРИЧИНУ, И ОНА СТРУКТУРНАЯ. Путь «Обух» мерится 1.136
    // якоря против 1.691 у соседа по этажу выбора — и НЕ потому, что «Обух»
    // слабее: на полном дереве он бьёт 5.85 удара оружия раз в три секунды
    // (1.95 в секунду) против 9.0 раз в шесть у «Череполома» (1.50).
    //
    // ДЕЛО В ФОРМЕ ЦЕНЫ. `spendAll` — это СБРОС: он берёт то, что накопилось,
    // и потому не отнимает ни у кого. Кнопка с фиксированной ценой — это
    // ПОДПИСКА: 20 ярости каждые три секунды это 6.7 в секунду, и «Жажде» с
    // «Потрошащим взмахом» не остаётся. У класса, чьё узкое место — полоска,
    // фиксированная цена конкурирует с генератором, а сброс нет.
    //
    // Откат поэтому базовый ДЕСЯТЬ, а не шесть: ветка режет его вдвое
    // («Скорый череп»), и десять превращаются в пять — 4.0 ярости в секунду
    // вместо 6.7. Свести пару до равенства это всё равно не свело: узел
    // остаётся слабее соседа, и это записано в отчёте как открытый вопрос, а
    // не заглажено числом.
    id: 'blunt-edge',
    ...
    cooldownSec: 10,
    weaponDamagePercent: new Decimal(2.1),
    triggersGcd: true,
    weaponDamageFromResource: new Decimal(5),
  },
  {
    // РАСПРАВА БЕЗ КОНЦА — ВЕНЕЦ РЕЗНИ, ВЫДАННЫЙ ТАЛАНТОМ. Добивание, которое
    // ВОЗВРАЩАЕТ БОЛЬШУЮ ЧАСТЬ ЦЕНЫ и ходит по короткому откату: пока цели
    // умирают, цепочка держится сама.
    //
    // «НЕ УХОДИТ В ОТКАТ ПРИ УБИЙСТВЕ» — ЭТОГО НЕТ, И ВОТ ПОЧЕМУ. Сброс отката
    // ПО СОБЫТИЮ убийства тик поднять умеет, а модель боя — нет: убийств в
    // секунду это её собственный результат, и умение, чей темп зависит от её
    // результата, пришлось бы решать уравнением. Тот же довод закрыл событие
    // «убийство» в `ProcTrigger`. Короткий откат плюс `refund` дают тот же
    // РИТМ и считаются обеими сторонами одинаково.
    id: 'endless-reckoning',
    ...
    refund: { resourceShare: 0.6 },
  },
  {
    // МЯСНИК — ВЕНЕЦ ЖИЛ, ВЫДАННЫЙ ТАЛАНТОМ. Своя кровь идёт в ярость вдвое
    // щедрее «Кровавой платы», и тем же ударом герой входит в стойкость: с
    // каждым пропущенным ударом входящее смягчается сильнее.
    //
    // «БРОНЯ РАВНА НАНЕСЁННОМУ СЕБЕ УРОНУ» — ЭТОГО НЕТ. Прибавка, считаемая
    // от только что нанесённого урона, живёт одну секунду боя, а вход модели
    // — статы и моб: она такую прибавку не увидит. `resolve` даёт тот же
    // смысл («чем тяжелее бой, тем крепче стоишь») и считается обеими
    // сторонами одинаково.
    id: 'butcher',
    ...
    weaponDamagePercent: new Decimal(0.5),
    triggersGcd: true,
    bloodPrice: { hpShare: 0.12, resourceShare: 0.9 },
    resolve: { perHitTaken: 0.06, maxShare: 0.3, durationSec: 12 },
    autocast: { heroHpAbove: 0.7 },
  },
  {
    // ЗОВ — ВЕНЕЦ ЧУТЬЯ, ВЫДАННЫЙ ТАЛАНТОМ. Одно нажатие наливает полоску
    // доверху: у класса, чья единственная беда — пустая шкала в начале боя,
    // это и есть венец автономности.
    //
    // «КАЖДОЕ УБИЙСТВО СОКРАЩАЕТ ОТКАТ ВДВОЕ» — ЭТОГО НЕТ, по тому же доводу,
    // что и у «Расправы без конца»: сброс отката по убийству модель досчитать
    // не может — убийств в секунду это её собственный результат. Откат в
    // девяносто секунд назначен так, чтобы «Зов» приходился примерно на один
    // цикл боёв, а не был кнопкой на каждую схватку.
    id: 'call',
    icon: 'ability-call',
    name: 'Зов',
    type: 'instant',
