...
export const HERBS: HerbDef[] = [
  ...
  },
  {
    ...
    perMinute: 0.8,
    // БЫЛО 0.9, И ЭТО РОВНО ТА ГРАНИЦА, О КОТОРОЙ ГОВОРИТ КОНТРАКТ.
    //
    // Правило записано в шапке этого файла: запаса зоны хватает на почти
    // непрерывное действие ОДНОГО зелья и НЕ хватает на три сразу. При 0.9
    // нужда трёх склянок Затопленного яруса равнялась снабжению ровно
    // (0.3 склянки в минуту × три пучка = 0.9), то есть три помещались
    // впритык — контракт держался нулевым запасом и никем не проверялся.
    //
    // Проверка появилась в эту ночь (`potions.test.ts`) и сразу это нашла.
    // Ноль-восемь возвращает правилу смысл: одно зелье обеспечено с запасом,
    // три — нет.
  },
]
