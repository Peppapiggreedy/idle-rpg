...
// 1.8 вместо прежних 0.5 — ровно та же ЯРОСТЬ В АБСОЛЮТНЫХ ЧИСЛАХ. Ставка
// множит ИСХОДНЫЙ урон босса, а он срезан примерно в 3.6 раза (см. CHAIN);
// оставь 0.5 — и ярость перестала бы догонять отстающего по урону в секунду,
// то есть данж перестал бы быть воротами. Проверяется в обе стороны тестами
// лестницы: с положенной долей экипировки цепочка проходится, с долей на
// пятнадцать пунктов ниже — нет.
export const ENRAGE_GROWTH = 1.8
...
// одно и то же дважды.
//
// УРОН СРЕЗАН ВТРОЕ И ЕЩЁ НЕМНОГО (было 1.05/1.28/1.5, до того 1.4/1.7/2.0),
// и это ПРЯМОЕ СЛЕДСТВИЕ контракта цены боя. Множитель считается от урона
// ОБЫЧНОГО моба того же уровня, а он вырос почти вчетверо: боссы поехали
// следом и стали убивать героя своего уровня входа за первую же схватку.
//
// ПОЧЕМУ МНОЖИТЕЛЬ УПАЛ НИЖЕ ЕДИНИЦЫ, то есть босс бьёт слабее рядового моба
// ЗА УДАР. Потому что «злее» меряется ЦЕПОЧКОЙ, а не ударом, — ровно как
// метка безопасности зоны считает бой целиком, а не худший удар. Бой с боссом
// идёт около минуты против дюжины секунд с обычным мобом, и трёх схваток
// подряд привал не разделяет: при равном уроне за удар цепочка стоила бы
// пятнадцати обычных боёв, а всего запаса героя хватает на четыре. Данж
// остаётся данжем не силой удара, а длиной без передышки и отметкой ярости;
// тест «цепочка дороже трёх обычных боёв» держит именно это.
//
// ЧИСЛА ЗДЕСЬ ХРУПКИЕ, и это стоит знать заранее: за минуту боя герой
// восстанавливает около 83% своего запаса (реген в бою — 1/70 запаса в
// секунду), поэтому исход схватки решает РАЗНОСТЬ входящего урона и регена,
// а не сам урон. Отсюда узкая полоса: 0.28 от прежнего проходится всеми
// восемью данжами, 0.32 — уже нет. Настоящее лекарство — короткий бой с
// боссом (его запас HP) или меньший реген в бою; и то и другое отдельное
// решение, и в этой правке оно не принято.
...
export const TIER_GROWTH = {
  ...
  // 1.07 вместо 1.04: базовый урон боссов срезан втрое с лишним, и прежняя
  // ставка перестала догонять рост силы героя к концу лестницы — ворота
  // шестого, седьмого и восьмого данжей открывались кому попало. Подобрано
  // замером и держится тем же двусторонним тестом лестницы.
  damage: 1.07,
  reward: 1.12,
