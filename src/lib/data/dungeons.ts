...
//
// МНОЖИТЕЛИ ПОДЕЛЕНЫ НА 1.20 (2.35 → 1.96, 1.62 → 1.35, 1.26 → 1.05) РОВНО НА
// СТОЛЬКО, НА СКОЛЬКО ПОДНЯЛСЯ УРОН ОБЫЧНОГО МОБА (третья ночь, стадия 3:
// MONSTER_BASE.damage 15.3 → 18.36). Множитель считается ОТ базы, поэтому без
// этого деления боссы подорожали бы заодно с рядовыми мобами — а контракт у
// них СВОЙ и отдельный: схватка стоит около 80 % запаса, и он был настроен
// под лечение. Абсолютный урон боссов не изменился ни на единицу; изменилось
// только то, от чего он считается.
//
// МНОЖИТЕЛИ ПОДНИМАЛИСЬ НА ТРЕТЬ И БОЛЬШЕ (1.74 → 2.35, 1.16 → 1.62,
// 0.89 → 1.26) С ПОЯВЛЕНИЕМ ЛЕЧЕНИЯ У СТРАЖА (вторая ночь, стадия 3).
// Схватка с боссом длится 25–65 секунд, и автокаст успевает вылечиться два-
// четыре раза (откат 12 с) — по четверти запаса каждый: с прежними числами
// цена схватки упала с 74–94 % до 31–63 %, и ворота открылись всем. Числа
// подобраны сканом тика по всем восьми тирам с лечением: первые два босса
// ×1.35, третий ×1.42 — у него бой длиннее, лечений больше, и без своей
// поправки он стоил бы 53–59 % на средних тирах. Замер с лечением: 62–91 %
// по цепочкам, контракт 60–95 держится. Порог выживания РЯДОМ: при ×1.5 герой
// положенной доли гибнет на первом же боссе — лечение ограничено откатом, а
// не маной, и добавить ему нечего. БЕЗ ЛЕЧЕНИЯ (снятая галка, класс без
// лечащего умения) босс теперь стоит больше запаса: данж для Стража — место,
// где лечение обязательно, и это осознанно.
//
// МНОЖИТЕЛИ ПОДЕЛЕНЫ ЕЩЁ НА 1.053 (2.35/1.20 = 1.96 -> 1.86, 1.35 -> 1.28,
// 1.05 -> 1.00) С ПОЯВЛЕНИЕМ БРОНИ. Множитель считается от урона обычного
// моба, а тот вырос вдвое с лишним под броню (MONSTER_BASE.damage 18.36 ->
// 38.87), — значит абсолютный урон боссов вырос ровно во столько же, а броня
// у эталонного героя срезает около половины. Наивно это должно было
// уравняться само; на деле схватка подорожала: цена — это «входящий минус
// реген минус лечение», а лечение и реген от правки не сдвинулись, и вся
// прибавка легла на разность. Замер тиком (24 схватки, восемь данжей):
//   без поправки → 63-97 %, в среднем 85, тир 2 за верхней границей
//   ×0.89       → 54-81 %, в среднем 68, первый данж под нижней границей
//   ×0.95       → 63-92 %, в среднем 78   ← выбрано: середина коридора 60-95
// Разброс 63-92 шире прежних 69-89 и упирается в обе границы почти поровну —
// это и есть признак, что дальше двигать множитель некуда: любое смещение
// выталкивает противоположный край.
//
// ПОЧЕМУ УРОН ПАДАЕТ ОТ ПЕРВОГО БОССА К ТРЕТЬЕМУ (1.86 -> 1.28 -> 1.00). Это
...
//
// ОТМЕТКИ ЯРОСТИ 48 / 61 / 79 (были 50 / 64 / 82). Контракт «отметка
// достижима, а не нарисована» требует, чтобы герой ровно с положенной долей
// экипировки её ЗАДЕВАЛ: точная модель боя (мана с привала) укоротила его
// схватку на два-три процента, и отметка на трёх данжах оказалась на 1-2 %
// дальше боя. Отметки подвинуты на те же проценты; цену схватки (тик) это не
// меняет — до отметки герой положенной доли не доживает, а отставший
// упирается в неё чуть раньше.
...
export const TIER_GROWTH = {
  ...
  // к экипировке (requiredGearShare) и уровень боссов.
  //
  // 1.012 — ПОСЛЕ ТОЧНОЙ МОДЕЛИ БОЯ. Единица держалась на модели, которая не
  // знала, что привал наливает ману: она занижала урон героя на 10-15 %, и
  // лестница ворот сходилась на этом занижении. Как только модель стала
  // совпадать с тиком (±3 % по убийствам в час), герой с экипировкой на
  // пятнадцать пунктов ниже положенной стал проходить шестой-восьмой данж
  // с запасом 0-6 %. Ставка 1.2 % за тир (восьмой — +8.7 %) возвращает
  // ворота: тик даёт схватке восьмого тира 82 % запаса до правки и около
  // 89 % после, под потолком контракта в 95 %. Первый тир не трогается.
  damage: 1.012,
  reward: 1.12,
