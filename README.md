...
Различаются они ЦЕНОЙ ОШИБКИ, а не только временем. Все числа — замер на
четырёх ядрах, а не оценка; чем именно платит каждый ярус и чего он НЕ
ловит, подробно — в `CLAUDE.md`, раздел «Какие проверки когда».

| ярус | команда | идёт | когда гонять |
| --- | --- | ---: | --- |
| статика контента | `content:check` | 5 с | после правки `src/lib/data/` |
| быстрый набор | `test` | 49 с | на каждой правке |
| прибор веток | `test:branches` | 185 с | после правки дерева талантов |
| проба баланса | `test:balance:sample` | 22.5 мин | после правки чисел баланса |
| полная матрица | `test:balance` | 47 мин | один раз перед PR |
