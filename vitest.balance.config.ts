 ...
 */
/**
 * ЧИСЛА СНЯТЫ С ПРОГОНА, А НЕ НАЗНАЧЕНЫ. Сумма длительностей ТЕСТОВ файла в
 * полной матрице от 10 сентября (13 файлов, стена 2809 с, работы 8731 с).
 * Прежняя таблица врала кратно: `run.test.ts` стоял как 204 с при
 * измеренных 2020, `gold.test.ts` — 543 при 1464, `power-budget.test.ts` —
 * 24 при 49. Ошибка не безобидная: очередь ставила самые дорогие файлы в
 * хвост, а раннер раскладывает по ядрам именно ФАЙЛЫ.
 *
 * ЧЕГО ЗДЕСЬ НЕТ — ВРЕМЕНИ ИМПОРТА, и это надо знать, читая таблицу.
 * `power-budget.test.ts` идёт 306 секунд по часам при 49 секундах тестов:
 * остальное — эталонное прохождение класса, которое строится при загрузке
 * модуля (109 с на класс, замер стадии 1 в docs/CHECKS.md). Со снятой
 * изоляцией эту цену платит ПЕРВЫЙ файл воркера, а не каждый, поэтому она и
 * не является свойством файла — приписать её сюда значило бы соврать иначе.
 *
 * `run.test.ts` — отдельный случай: 2020 с и упёртый срок 1800 с на таблице
 * пути. Причина известна с ночи Псаря: путь Псаря застревает на шестнадцатом
 * уровне на двух сидах из трёх и идёт до потолка в 30 игровых часов. Это
 * УНАСЛЕДОВАННОЕ КРАСНОЕ, а не регрессия ночи проверок (docs/CHECKS.md).
 */
const COST_SECONDS: Record<string, number> = {
  'run.test.ts': 2020,
  'gold.test.ts': 1464,
  'balance-progress.test.ts': 884,
  'balance-zones.test.ts': 776,
  'balance-abilities.test.ts': 391,
  'balance-pacing.test.ts': 283,
  'balance-model.test.ts': 251,
  'balance-key-choices.test.ts': 116,
  'balance-style.test.ts': 111,
  'power-budget.test.ts': 49,
  'balance-temple.test.ts': 6,
  'dungeon-ladder.test.ts': 1,
  'level-gap.test.ts': 1,
}
...
export default defineConfig({
  test: {
    ...
    /**
     * ОБЩИЙ ПОЛ ДЛЯ СРОКОВ. Умолчание vitest — пять секунд, и оно писалось не
     * под тесты, где один замер это часы игрового времени. Свои сроки у тяжёлых
     * тестов стоят рядом с ними и этот пол перебивают; он нужен тем, у кого
     * своего срока нет, — чтобы занятая машина не роняла их вместо поломки.
     */
    testTimeout: 120_000,
