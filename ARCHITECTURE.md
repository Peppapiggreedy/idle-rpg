...
|---|---|---|
| `src/main.ts` | четыре ветки: `debugRoute()` даёт `'balance'` / `'ui'` / null, `presetName()` — режим съёмки. На отладочных страницах и в режиме съёмки игровой цикл не запускается и сейв не трогается. Каждая ветка ставит `data-ready` на `<html>` — по нему Playwright понимает, что страница готова | `App.svelte`, `ui/BalancePage.svelte`, `ui/ShowcasePage.svelte`, `ui/route`, `ui/preset`, `stores/game` |
...
какое заранее заданное состояние просят снять. Всё требует `?debug=1`.
...
`playwright.config.ts` собирает игру, поднимает `vite preview` и снимает
одиннадцать кадров: три пресета × три ширины (390 / 768 / 1280), витрина `/ui`
и точный снимок значений токенов. Эталоны — `tests/screenshots/`,
обновляются `npm run screenshots:update`.
