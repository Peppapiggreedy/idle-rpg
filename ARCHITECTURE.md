...
| Опыт растёт отдельно от золота | `data/monsters.ts` | золото платит за силу и обязано успевать за геометрически дорожающей заточкой; опыт — это ЧАСЫ игрока. На двадцати ступенях герой уходит на восемь зон вперёд к десятому уровню, и при общей ставке уровень значил бы «как глубоко залез», а не «сколько сыграл» |
