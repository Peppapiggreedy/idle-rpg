...
звук, три ветки талантов, двадцать зон, второй класс, профессии и
...
| HP, урон и награда мобов растут ЛИНЕЙНО | `data/monsters.ts` | сила героя растёт линейно и иначе не может: она на вещах, а сила вещи линейна от уровня зоны. Экспонента в HP обгоняла бы прямую при любых числах |
...
| Заточка снесена, сила героя — на вещах | `data/balance.ts`, `game/loot.ts` | у предмета есть УРОВЕНЬ (уровень моба, с которого упал), и сила вещи растёт от него линейно — той же прямой, что HP мобов. Прямая против прямой: отношение сил внутри зоны почти постоянно, и коридор темпа держится по построению |
| Четыре атрибута — статы конвейера | `game/stats.ts` | сила — урон, ловкость — скорость и криты, интеллект — мана, живучесть — здоровье. Атрибут проходит конвейер как обычный стат, а его итог разворачивается во вклады плоскими модификаторами `attribute:<имя>` — раскладка на панели статов показывает их построчно |
| Утечка ярости — доля запаса | `data/classes.ts` | как и доход с удара: с плоской утечкой глубокий изувер копил бы ярость впрок. Интеллект изуверу чужд — гасится множителем класса, и запас ярости не дешевеет к концу лестницы |
| Двадцать зон по пять уровней мобов | `data/zones.ts` | полосы 1-5, 6-10 … 96-100 идут подряд: у каждого уровня мобов есть своя зона, дыр в мире нет. Проверяется данными, а не договорённостью |
| Опыт растёт отдельно от золота | `data/monsters.ts` | золото — экономика (ремёсла, сбросы талантов), опыт — ЧАСЫ игрока. На двадцати ступенях герой уходит на восемь зон вперёд к десятому уровню, и при общей ставке уровень значил бы «как глубоко залез», а не «сколько сыграл» |
| Имя ресурса — ТЕКСТ | `ui/resource.ts` | данные говорят, что ресурс копится от ударов и тает вне боя; «ярость» это или «мана» — вопрос языка, и решать его логике нечем. Оттуда же заголовок полоски, цена умения и три строки в статах |
