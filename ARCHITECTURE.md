...
|---|---|---|
| `src/main.ts` | три страницы: `debugRoute()` возвращает `'balance'` / `'ui'` / null; на отладочных монтируется только их компонент (игровой цикл не запускается и сейв не трогается), иначе `initGame()` → mount App → `startGameLoop()` + слушатель `visibilitychange` | `App.svelte`, `ui/BalancePage.svelte`, `ui/ShowcasePage.svelte`, `ui/route`, `stores/game` |
| `src/App.svelte` | компоновка экрана: две колонки от 720px, одна ниже. Никакой логики | все компоненты `ui/` |
...
### `src/lib/ui/` — дизайн-система, 17 компонентов и модуль маршрута
...
страница открыта (`'balance'`, `'ui'` или null). Требует `?debug=1` и понимает
два адреса — чистый путь `/idle-rpg/ui` (его отдаёт GitHub Pages через
