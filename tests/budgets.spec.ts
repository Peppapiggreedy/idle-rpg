...
test('за полчаса игрового времени сцена и DOM не разрастаются', async ({ page }) => {
  ...
  // ПОЛНОЙ СУМКИ МАЛО: ЕСТЬ ТРЕТЬЯ ЗАКОННО РАСТУЩАЯ ВЕЩЬ — ЛЕСТНИЦА ОТКРЫТИЙ.
  // Панель механики, до уровня которой герой не дошёл, теперь не отображается
  // ВОВСЕ (а не серой с замком), и каждый взятый порог добавляет в разметку
  // целую панель — таланты, ремёсла, зачарование, задания. Это сотни узлов, и
  // это не утечка.
  //
  // Сумка и лестница набираются НЕЗАВИСИМО, и кто успеет первым — вопрос
  // рулетки находок: замер видел и 693 узла в точке отсчёта (сумка успела
  // раньше открытий), и 1070 (открытия успели раньше). В первом случае тест
  // падал на законном росте — 693 → 1068 при потолке в пять процентов.
  //
  // Поэтому точка отсчёта берётся по САМОМУ УСТАНОВИВШЕМУСЯ СОСТОЯНИЮ, а не
  // по одному из его признаков: ждём, пока число узлов перестанет расти между
  // двумя замерами подряд. Утечку это не прячет — она как раз расти и не
  // перестанет, и тогда ожидание упрётся в свой потолок и скажет об этом.
  const settleStep = 5_000
  let settled = await domNodes(page)
  let stable = false
  for (let waited = 0; waited < 120_000; waited += settleStep) {
    await page.waitForTimeout(settleStep)
    const next = await domNodes(page)
    if (next <= Math.ceil(settled * 1.01)) { settled = next; stable = true; break }
    settled = next
  }
  expect(
    stable,
    'DOM не перестал расти за две минуты после полной сумки — это уже похоже на утечку',
  ).toBe(true)
